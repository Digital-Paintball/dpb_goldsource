...
void CHudSpectator::SetModes(int iNewMainMode, int iNewInsetMode)
{
	...
	if ( iNewMainMode != g_iUser1 )
	{
		...
		if ( !gEngfuncs.IsSpectateOnly() )
		{
			gEngfuncs.pfnServerCmd(UTIL_VarArgs("specmode %i\n",iNewMainMode));
			return;
		}
...
bool CHudSpectator::AddOverviewEntityToList(HSPRITEZ sprite, cl_entity_t *ent, double killTime)
{
	for ( int i = 0; i< MAX_OVERVIEW_ENTITIES; i++ )
	{
...
{
	int newInsetMode = ((int)m_pip->value + 1) % (INSET_MAP_FREE);
