...
//
// $Workfile:     $
// $Date: 2004/12/31 15:26:23 $
//
//-----------------------------------------------------------------------------
// $Log: vgui_TeamFortressViewport.cpp,v $
// Revision 1.4  2004/12/31 15:26:23  jsherman
// big commit
//
// Revision 1.3  2004/11/13 20:01:23  jsherman
// crash fixes
//
// Revision 1.2  2004/11/13 03:16:31  jsherman
// client changes from jeff
//
// Revision 1.1  2004/01/04 23:54:07  jonah
// add client
//
//
// $NoKeywords: $
...
char *WPNnames[WPN_CATEGORIES][WPN_WEAPONS]= 
{
...
char *WPNfnames[WPN_CATEGORIES][WPN_WEAPONS]=
{
...
void TeamFortressViewport::Initialize( void )
{
	...
	if (m_pConfigMenu)
	{
		m_pConfigMenu->setVisible( false );
	}
	if (m_pScoreBoard)
	{
...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if ( g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission )	// don't draw in dev_overview mode
	{
		...
		int timer = (int)( gHUD.m_Timer.endtime - gHUD.m_flTime );
...
bool TeamFortressViewport::AllowedToPrintText( void )
{
	...
	if ( m_pCurrentMenu && g_iPlayerClass == 0 )
	{
		...
		if ( iId == MENU_TEAM || iId == MENU_CLASS || iId == MENU_INTRO || iId == MENU_CLASSHELP || iId == MENU_FIRSTMENU)
			return FALSE;
	}
...
// OUR FIRST MENU
//======================================================================================
CMenuPanel* TeamFortressViewport::ShowConfigMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	m_pConfigMenu->Reset();
	return m_pConfigMenu;
}

void TeamFortressViewport::CreateConfigMenu()
{
	// Create the panel
	m_pConfigMenu = new CConfigMenu(100, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pConfigMenu->setParent(this);
	m_pConfigMenu->setVisible( false );
}
...
int TeamFortressViewport::MsgFunc_ScoreInfo( const char *pszName, int iSize, void *pbuf )
{
	...
	if ( cl > 0 && cl <= MAX_PLAYERS )
	{
		...
		if(cl==gEngfuncs.GetLocalPlayer()->index) {
			g_hud_r=colors[teamnumber][0];
			g_hud_g=colors[teamnumber][1];
			g_hud_b=colors[teamnumber][2];
...
int TeamFortressViewport::MsgFunc_Credits( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ(pbuf,iSize);
	if(m_pConfigMenu)
		m_pConfigMenu->setMaxCredits(READ_BYTE());
	return 1;
}
...
int TeamFortressViewport::MsgFunc_Respawn( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	int cl = READ_BYTE();
