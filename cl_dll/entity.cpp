...
/*	switch ( type )
	...
	if ( g_iUser1 )
	{
	...
	}/* else if(ent->index>0&&ent->index<=gEngfuncs.GetMaxClients()&&g_PlayerExtraInfo[ent->index].teamnumber!=g_iTeamNumber) {
		int i,idx=ent->curstate.usehull;
		float tmp[3],f[3],r[3],u[3];
		pmtrace_t tr;
		tmp[0]=tmp[2]=0.0f;
		tmp[1]=ent->angles[1];
		...
		for(i=7;i>=0;i--) {
			tmp[0]=ent->origin[0]+f[0]*offsets[idx][i][0]+r[0]*offsets[idx][i][1]+u[0]*offsets[idx][i][2];
			tmp[1]=ent->origin[1]+f[1]*offsets[idx][i][0]+r[1]*offsets[idx][i][1]+u[1]*offsets[idx][i][2];
			tmp[2]=ent->origin[2]+f[2]*offsets[idx][i][0]+r[2]*offsets[idx][i][1]+u[2]*offsets[idx][i][2];
			gEngfuncs.pEventAPI->EV_PlayerTrace(v_origin,tmp,PM_NORMAL|PM_GLASS_IGNORE,-1,&tr);
			if(tr.fraction==1.0f||gEngfuncs.pEventAPI->EV_IndexFromTrace(&tr)==ent->index) {
				gEngfuncs.pEventAPI->EV_PopPMStates();
				return 1;
			}
		//	gEngfuncs.Con_Printf("HIT ENT: %i\n",gEngfuncs.pEventAPI->EV_IndexFromTrace(&tr));
		}
		gEngfuncs.pEventAPI->EV_PopPMStates();
		return 0;
...
void DLLEXPORT HUD_TxferPredictionData ( struct entity_state_s *ps, const struct entity_state_s *pps, struct clientdata_s *pcd, const struct clientdata_s *ppcd, struct weapon_data_s *wd, const struct weapon_data_s *pwd )
{
	...
	if ( gEngfuncs.IsSpectateOnly() )
	{
		...
		pcd->iuser4 = g_iUser4; // second target
...
#endif
cl_entity_s g_FlagModels[MAX_PLAYERS];
void DrawFlags()
{
  ...
  for(int i=0; i < MAX_PLAYERS; i++)
  {
    ...
    cl_entity_s *pEnt = &g_FlagModels[iOutModel];
    ++iOutModel;
    ...
    pEnt->curstate.renderfx = kRenderFxNoDissipation;
		pEnt->curstate.rendercolor=c;
    pEnt->curstate.framerate = 1;
    pEnt->curstate.frame = 0;
    pEnt->model = (struct model_s*)gEngfuncs.GetSpritePointer(gHUD.m_Flag.FLG)
;
    pEnt->angles[0] = pEnt->angles[1] = pEnt->angles[2] = 0;
    pEnt->curstate.scale = 0.3f;
...
#define MODEL_FORWARD 13.0f //CVAR_GET_FLOAT("hej_f") //(20.0f)
#define MODEL_RIGHT -4.75f //CVAR_GET_FLOAT("hej_r") //(13.0f)
#define MODEL_UP 3.75f //CVAR_GET_FLOAT("hej_u") //(7.5)
...
void DrawModel(char *m) {
	...
	memset(&wmdl, 0, sizeof(wmdl));
	wmdl.model=gEngfuncs.CL_LoadModel(m,&model);
	if(model<0)
		return;
  if(strstr(m,"player")&&gViewPort) {
    wmdl.curstate.body=gViewPort->m_pConfigMenu->m_Mask->m_iOption;
    wmdl.curstate.skin=gViewPort->m_pConfigMenu->m_Jersey->m_iOption<<1;
  }
	wmdl.curstate.angles[0]=-v_angles[0];
	wmdl.curstate.angles[1]=v_angles[1]+rotang;
	if(wmdl.curstate.angles[1]>180.0f)
		wmdl.curstate.angles[1]-=360.0f;
	else if(wmdl.curstate.angles[1]<-180.0f)
		wmdl.curstate.angles[1]+=360.0f;
	VectorMA(v_origin,MODEL_FORWARD,f,wmdl.origin);
	VectorMA(wmdl.origin,MODEL_RIGHT,r,wmdl.origin);
	VectorMA(wmdl.origin,MODEL_UP,u,wmdl.origin);
	gEngfuncs.CL_CreateVisibleEntity(ET_NORMAL,&wmdl);
	
}
extern char vgui_Model[255];
void DLLEXPORT HUD_CreateEntities( void )
{
...
*/
	...
	if(gViewPort && gViewPort->m_pCurrentMenu && gViewPort->m_pCurrentMenu->GetMenuID() == 30)
		DrawModel(vgui_Model);
	rotang+=gHUD.m_flTimeDelta*90.0f;
	if(rotang>180.0)
		rotang-=360.0f;
	DrawFlags();
...
// Experiment
			...
			if ( pTemp->flags & (FTENT_COLLIDEALL | FTENT_COLLIDEWORLD) )
			{
				...
				if ( pTemp->flags & FTENT_COLLIDEALL )
				{
					...
					gEngfuncs.pEventAPI->EV_PushPMStates();	
					gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );
					gEngfuncs.pEventAPI->EV_SetSolidPlayers(pTemp->clientIndex-1);
				...
				if ( traceFraction != 1.0 )	// Decent collision now, and damping works
				{
					float  proj, damp;
