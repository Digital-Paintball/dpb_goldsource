...
#define WPN_CATEGORIES 3
#define WPN_WEAPONS    9
extern int sTFValidClassInts[];
extern char *WPNnames[WPN_CATEGORIES][WPN_WEAPONS];
extern char *WPNfnames[WPN_CATEGORIES][WPN_WEAPONS];
extern char *CATnames[WPN_CATEGORIES];
...
private:
	...
	void		 CreateConfigMenu( void );
	CMenuPanel*	 ShowConfigMenu( void );
...
public:
	...
	int		CreateCommandMenu ( char *menuFile, int direction, int yOffset,bool flagDesign, float flButtonSizeX, float flButtonSizeY, int xOffset);
	...
	bool IsPaintballHudVisible();
	void ShowPaintballHud();
	...
	int	MsgFunc_Credits( const char *pszName, int iSize, void *pbuf );
	int MsgFunc_Respawn( const char *pszName, int iSize, void *pbuf );
...
public:
	...
	virtual void actionPerformed(Panel* panel)
	{
		...
		if ( !strncmp( m_pszCommand, "jointeam",8 ) )
			g_bPickedTeam = 1;
	}
};
	...
	SpectToggleButton(const char *cvarname,const char* text,int x,int y,int wide,int tall,bool flat) :
	  ...
	  virtual void paint()
	  {
		  if(isArmed())
			  ...
			  setFgColor(143,143,54,15);
		   pLabelOn->setVisible(m_cvar&&m_cvar->value);
		   Button::paint();
	  }
};
//============================================================
...
//DPB BUY MENU CLASS
//=====================================================================
#define PAINT_COLORS 7
class ComboBox;
class CConfigMenu : public CMenuPanel
{
private:
...
public:
	...
	ComboBox *m_Mask;
	void Pod(int i);
	void setMaxCredits(int c);
	void SendConfig();
	virtual void SetActiveInfo(int i);
	CConfigMenu(int iTrans, int iRemoveMe, int x, int y, int wide, int tall);
};
...
class CWeaponMenuPanel : public CMenuPanel
{
private:
	CTransparentPanel	*m_pPanels[WPN_WEAPONS];
	CommandButton		*m_pButtons[WPN_WEAPONS];
	ScrollPanel			*m_pScrollPanel;

	CImageLabel			*m_pImages[WPN_WEAPONS];

	int					m_iCurrentInfo;

public:
	CWeaponMenuPanel(int iTrans, int iRemoveMe, int x,int y,int wide,int tall,int cat);
