...
int __MsgFunc_Credits(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_Credits( pszName, iSize, pbuf );
	return 0;
}
...
int __MsgFunc_Respawn(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_Respawn( pszName, iSize, pbuf );
	return 0;
}
...
void CHud :: Init( void )
{
	...
	CVAR_CREATE("hej_u","0",0);
	CVAR_CREATE("hej_r","0",0);
	CVAR_CREATE("hej_f","0",0);
	...
	if ( !m_pSpriteList )
	{
		...
		if (m_pSpriteList)
		{
			...
			// allocated memory for sprite handle arrays
 			m_rghSprites = new HSPRITEZ[m_iSpriteCount];
