...
BOOL CFuncTank :: StartControl( CBasePlayer *pController )
{
	...
	if ( m_pController->m_fDeployed )
		m_pController->m_weapon->Holster();
...
void CFuncTankGun::Fire( const Vector &barrelEnd, const Vector &forward, entvars_t *pevAttacker )
{
	...
	if ( m_fireLast != 0 )
	{
		...
		if ( bulletCount > 0 )
		{
			...
			{
				vec3_t dir=forward+Vector(
					g_engfuncs.pfnRandomFloat(-gTankSpread[m_spread][0]/2,gTankSpread[m_spread][0]/2),
					g_engfuncs.pfnRandomFloat(-gTankSpread[m_spread][1]/2,gTankSpread[m_spread][1]/2),
					g_engfuncs.pfnRandomFloat(-gTankSpread[m_spread][2]/2,gTankSpread[m_spread][2]/2));
			//	FirePaintball((float*)&barrelEnd,(float*)&forward,entindex()-1);
			}
